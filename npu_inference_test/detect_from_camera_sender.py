...
if __name__ == '__main__':
    ...
    target = 'rk3566'
    img_counter = 1
    img_name = 'rknn_detect_from_camera_'
    ...
    # run test
    sender = imagezmq.ImageSender(connect_to='tcp://192.168.186.94:5555')

    sender_hostname = socket.gethostname() # send the board's hostname with each image
    cap = cv2.VideoCapture(0)
    time.sleep(2)  # allow camera sensor to warm up

    while True:  # send images as stream until Ctrl-C
        ret, frame = cap.read()
        img_name = img_name + str(img_counter) + '.jpg'
        ...
        sender.send_image(sender_hostname, img_p)
